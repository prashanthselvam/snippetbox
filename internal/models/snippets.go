...
// Define a Snippet type to hold the data for an invidual snippet. Notice how
...
func (m *SnippetModel) Get(id int) (Snippet, error) {
	...
	// placeholder parameter. This returns a pointer to a sql.Row value
	// which holds the result from the database.
	row := m.DB.QueryRow(stmt, id)

	// Initialize a new zeroed Snipper struct.
	var s Snippet

	// Use row.Scan() to copy the values from each field in sql.Row to the
	// corresponding field in the Snippet struct. Notice that the arguments
	// to row.Scan are *pointers* to the place you want to copy the data into,
	// and the numbers of arguments must be exactly the same as the number of
...
func (m *SnippetModel) Latest() ([]Snippet, error) {
	...
	// Initialize an empty slice to hold the Snippetstructs.
	...
	// call this - don't assume the iteration completed successfully over the entire
	// result set.
